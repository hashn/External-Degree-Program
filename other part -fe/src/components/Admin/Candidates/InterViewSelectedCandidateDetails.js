...
function InterViewSelectedCandidateDetails() {
  const [candidateList,setCandidateDetails]=useState([]);
  const [interviewCandidate,setIWSelectedCandidate]= useState([]);
  const statusA=" SELECTED  FOR COURSE";
  const [exammarks,setExamMarks]=useState([]);
    ...
    useEffect(()=>{
    axios.get(`http://localhost:3001/interviewselectedcandidatesDetailsId/${id}`).then((response)=>
   {
    setIWSelectedCandidate(response.data);
   ...
   axios.get(`http://localhost:3001/CandidatesDetailsId/${id}`).then((response)=>
        ...
        axios.get(`http://localhost:3001/examMarks/${id}`).then((response)=>
        {
...
const sletectCandidateForCourse =()=>{
    ...
    return (
        <div className="container border mt-4" style={{
            ...
            <h1>Interview Selected Candidates</h1>
            {interviewCandidate.map((val,key)=>{
                return(
                    <div key={val.ID}>
                                    <p className="border border-dark">ID:{val.ID}</p>
                                    <p className="border border-dark">Interview Marks:{val.InterviewMarks}</p>

                    </div>
                )

            })}
            {exammarks.map((val,key)=>{
                return(
                    <div key={val.ID}>
          ...
          {candidateList.map((val,key)=>{
              return(
                       <div>
                        ...
                        }} onClick={()=>handledelete(val.ID)} type="button">Delete</button></Link>
                        <Link to={"/UpdateInterviewSelectedCandidate/"+val.ID}><button 
                        type="button"
                        style={{
                            borderRadius:"25px",
                            color:"#ffff",
                        ...
                        onClick={()=>{sletectCandidateForCourse(val.ID)}}
                        >send email for course selection</button></Link>
                        </div>
                        <div style={{
                           
                            marginTop:"50px"
                        }}>
                        <Link to={"/InterviewSelectedCandidate/"+val.AccadamicYear}><button type="button" ><img 
...
export default InterViewSelectedCandidateDetails;
